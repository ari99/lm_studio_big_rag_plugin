/**
 * Pure chunking benchmark - isolates chunking performance from file I/O
 * Tests raw text chunking speed with pre-loaded content
 */
...
async function benchmarkChunkingParallel(
  ...
  const totalChunks = allChunks.reduce((sum, c) => sum + c.length, 0);
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════╗");
  console.log("║        Pure Chunking Benchmark (No File I/O)             ║");
  console.log("╚══════════════════════════════════════════════════════════╝");
  console.log();
  ...
  console.log("Loading test files into memory...");
  const tsFiles = await import("../ingestion/fileScanner").then(m => m.scanDirectory(DOCUMENTS_PATH));
  
  // Load content of first 50 text/markdown files
  ...
  for (const file of tsFiles) {
    ...
    if (file.extension === ".txt" || file.extension === ".md") {
      try {
        ...
        if (content.length > 1000) { // Skip very short files
  ...
  const reportPath = path.join(__dirname, "../../benchmarks/pureChunkReport.json");
