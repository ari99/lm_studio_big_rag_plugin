...
import { scanDirectory as nativeScanDirectory, isNativeAvailable } from "../src/native";
...
async function walk(dir: string, files: ScannedFile[]): Promise<void> {
...
async function scanDirectoryTS(rootDir: string): Promise<ScannedFile[]> {
...
async function runBenchmarks() {
  ...
  try {
    ...
    if (nativeAvailable) {
      const nativeSmallResult = await benchmark("Rust Native", () => nativeScanDirectory(smallDir));
    ...
    if (nativeAvailable) {
      const nativeMediumResult = await benchmark("Rust Native", () => nativeScanDirectory(mediumDir));
    ...
    if (nativeAvailable) {
      const nativeLargeResult = await benchmark("Rust Native", () => nativeScanDirectory(largeDir));
    ...
    if (nativeAvailable) {
      const nativeWideResult = await benchmark("Rust Native", () => nativeScanDirectory(wideDir));
