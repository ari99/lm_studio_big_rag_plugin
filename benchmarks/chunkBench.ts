...
import { chunkText as nativeChunkText, isNativeAvailable } from "../src/native";
...
async function runBenchmarks() {
  ...
  for (const { name, words } of testConfigs) {
    ...
    const tsResult = await benchmarkChunk(
      `TypeScript (${name})`,
      tsChunkText,
      testText,
      chunkSize,
    ...
    if (isNativeAvailable()) {
      ...
      const nativeResult = await benchmarkChunk(
        `Native Rust (${name})`,
        nativeChunkText,
        testText,
        chunkSize,
  ...
  for (const result of results) {
    console.log(
      `${result.name.padEnd(20)} | Avg: ${result.avgTimeMs.toFixed(2)}ms | Chunks: ${result.chunksProduced} | Ops/sec: ${result.opsPerSec.toFixed(1)}`
    );
  }
